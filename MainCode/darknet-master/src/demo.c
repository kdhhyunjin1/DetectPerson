...
mat_cv* det_img;
mat_cv* show_img;
mat_cv* init_img; //최초의 이미지
int image_width, image_height; //이미지 넓이, 높이
    ...
    int benchmark, int benchmark_layers)
{
    ...
    det_img = in_img;
    det_s = in_s;
    init_img = in_img; //원본 이미지를 결과이미지에 복사
    ...
    while (1) {
        ++count;
        {
            ...
            if (!benchmark) draw_detections_cv_v3(show_img, local_dets, local_nboxes, demo_thresh, demo_names, demo_alphabet, demo_classes, demo_ext_output); //영상에 예측 결과를 그려놓음
            free_detections(local_dets, local_nboxes);

            printf("\nFPS:%.1f \t AVG_FPS:%.1f\n", fps, avg_fps);


            if (!prefix) { //그 영상을 화면에 띄워줌
                if (!dont_show) {
                    show_image_mat(show_img, "Demo");
                    image_width = get_width_mat(show_img);
                    image_height = get_height_mat(show_img);
                    int c = wait_key_cv(1);
                    if (c == 10) {
    ...
    //표시
    create_window_cv("result", 0, 1352, 1013);
    draw_result(init_img, 1352, 1013);
    show_image_mat(init_img,"result");
