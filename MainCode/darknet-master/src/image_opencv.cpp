...
#endif
int WEIGHTS[1920 + 10][1080 + 10]; //사람 위치를 표시. 오래있을 수록 수치가 높아짐.
struct DATA {
    int x, y, value;
};
std::vector<DATA> people; //제일 오래 있었던 위치를 출력하기 위한 벡터

bool compare(const DATA& i, const DATA& j) { //정렬 순서 값으로 내림차순. 값이 같다면 좌표로 오름차순.
    if (i.value == j.value) {
        if (i.x == j.x) {
            return i.y < j.y;
        }
...
// ====================================================================
    ...
    */
        ...
        try {
            ...
            for (i = 0; i < num; ++i) {
                ...
                if (class_id >= 0) {
                    ...
                    //표시
                    if (strncmp(labelstr, "person", 6) == 0) { //만약 사람이라면
                        //중심에 가중치를 둠
                        int x = (left + right) / 2;
                        int y = (top + bot) / 2;
    ...
    void draw_result(mat_cv* mat, int width, int height) { //결과 출력 함수
        cv::Mat* img = (cv::Mat*)mat;

        for (int i = 0; i < height; i++) { //가중치에 따라 색깔을 설정. 사람이 오래 있을 수록 빨강->노랑->하양순으로 색깔이 바뀜
            for (int j = 0; j < width; j++) {
                if (WEIGHTS[i][j] != 0) {
                    ...
                    pt2.y = i + 1;
                    int rec = WEIGHTS[i][j];
                    people.push_back({ pt1.x, pt1.y, rec });
                    cv::Scalar color;
                    if (rec <= 155) {
                        color.val[0] = 0;
                        color.val[1] = 0;
                        color.val[2] = 100 + (double)rec;//빨강
                    }
                    else if (rec <= 155 + 255) {
                        color.val[0] = 0;
                        color.val[1] = (double)rec - 155;//초록
                        color.val[2] = 255;
                    }
                    else if (rec <= 155 + 510) {
                        color.val[0] = (double)rec - 255 - 155;//파랑
                        color.val[1] = 255;
                        color.val[2] = 255;
                    }
                    else {
                    ...
                    cv::rectangle(*img, pt1, pt2, color, CV_FILLED, 8, 0);
                }
            }
        }
        std::sort(people.begin(), people.end(), compare);
        printf("\n\nTop100 coordinate where people have been around for a long time.\n");
        printf("x: from the left, y: from the top\n\n");
        for (int i = 1; i <= people.size() && i <= 100; i++) {
            printf("Ranked%4d. x:%4d, y:%4d\n", i, people[i - 1].x, people[i - 1].y);
        }
    }
    // ----------------------------------------
